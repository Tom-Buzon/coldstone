...
func can_accept_formation_attack() -> bool:
	return weapon_available and _has_target() and state in [State.GUARD, State.APPROACH] and attack_cooldown <= 0.0 and _planar_target_distance() <= actor.combat_modifiers.intercept_reach
...
func formation_decision_tick(delta: float, attack_authorized: bool, moving_to_slot: bool) -> void:
	...
	if attack_authorized and can_accept_formation_attack():
		sortie_time += delta
		if sortie_time <= actor.combat_modifiers.sortie_seconds and actor.global_position.distance_to(_local_home()) < actor.combat_modifiers.intercept_reach:
			local_override = true
...
func formation_action_tick(delta: float) -> void:
	...
	if state == State.APPROACH and local_override and _has_target() and _planar_target_distance() > 1.7 and _target_facing_dot() > 0.45:
		local_motion = (target.global_position - actor.global_position).normalized() * actor.effective_move_speed()
		local_motion.y = 0.0
...
func _try_deliver_hit() -> void:
	...
	if offset.length() <= ATTACK_REACH and locked_direction.dot(offset.normalized()) >= 0.25:
		...
		if target.has_method(method):
			target.call(method, actor.effective_attack_damage() * (1.35 if heavy_cut else 1.0), actor, locked_direction)
