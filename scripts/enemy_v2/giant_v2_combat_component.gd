...
enum State { GUARD, APPROACH, WINDUP, RECOVERY, STUNNED, DEAD }
const PATTERNS: Array[Dictionary] = [
	...
	{"id": &"giant_slam", "windup": 1.7, "recovery": 2.15, "radius": 1.05, "dot": -1.0, "damage": 1.3},
]
...
func on_shield_blocked(_seconds: float) -> void:
	pass
...
func _begin_attack() -> void:
	current_pattern = PATTERNS[pattern_index % PATTERNS.size()]
	if actor.locomotion_factor < 1.0 and current_pattern.id == &"giant_slam":
	...
	if current_pattern.id == &"giant_slam":
		impact_center += locked_direction * actor.size_multiplier * 1.05
	var semantic: StringName = current_pattern.id
	var length := 1.1 if semantic == &"giant_punch" else (2.667 if semantic == &"giant_swipe" else 3.8)
...
func _deliver_impact() -> void:
	...
	if vertical > (1.15 if current_pattern.id == &"giant_slam" else actor.size_multiplier * 1.3) or vertical < -1.0:
		return
	...
	if not actor.get_world_3d().direct_space_state.intersect_ray(query).is_empty():
		return
	var damage: float = actor.definition.attack_damage * float(current_pattern.damage)
...
func _physics_process(delta: float) -> void:
	...
	actor.velocity.x = direction.x * actor.definition.move_speed * actor.locomotion_factor if moving else 0.0
	actor.velocity.z = direction.z * actor.definition.move_speed * actor.locomotion_factor if moving else 0.0
