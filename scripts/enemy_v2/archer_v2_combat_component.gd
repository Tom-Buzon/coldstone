...
func can_accept_formation_attack() -> bool:
	...
	return weapon_available and _has_target() and state in [State.GUARD, State.APPROACH] and retreat_remaining <= 0.0 and attack_cooldown <= 0.0 and distance >= MIN_RANGE and distance <= MAX_RANGE and line_clear and _army_lane_clear(target.global_position) and Projectile.can_spawn(get_tree())
...
func formation_action_tick(delta: float) -> void:
	...
	if state == State.APPROACH and retreat_remaining > 0.0:
		...
		if actor.global_position.distance_to(retreat_origin) < RETREAT_DISTANCE and actor.global_position.distance_to(_local_home()) < RETREAT_DISTANCE + 1.0:
			local_motion = retreat_direction * actor.definition.move_speed * 0.65
		else:
...
func _try_deliver_hit() -> void:
	...
	projectile.launch(origin, locked_aim, actor, target, actor.definition.attack_damage)
...
func _army_lane_clear(aim: Vector3) -> bool:
	...
	if runtime == null or not is_instance_valid(runtime) or not runtime.has_method("is_fire_lane_clear"):
		return true
	return bool(runtime.call("is_fire_lane_clear", actor.phalanx_group_id, actor.global_position, aim))
