...
func _init() -> void:
	...
	_assert(EnemyArchetypesScript.giant_ids().size() == 3, "Forge must expose exactly three giant tiers")
	for giant_id: StringName in EnemyArchetypesScript.giant_ids():
		...
		_assert(bool(profile.get("forge_default_match_perfect_hitbox", false)), "%s must default to climbable hitboxes" % giant_id)
		_assert(StringName(profile.get("giant_traversal_mode", &"")) == &"assisted", "%s must default to assisted traversal" % giant_id)
	...
	_assert(report.mob_count == 311, "population accounting must include deferred and giant spawn groups")
